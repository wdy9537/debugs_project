...
<%@ include file="/views/common/headerView.jsp" %>
 ...
 <div class="loginbox">
            ...
            <h2>비밀번호 찾기</h2>
           <form id="enroll-form"  action="<%=contextPath %>/find_Pwd.me" method="post">
           
                <!--작성하지 않아도 문제는 없음-->
                <fieldset>
                    <legend>비밀번호 찾기 구역</legend>
                    <label for="loginid">아이디</label>
                    <input type="text" id="loginid"  name="userId" placeholder="아이디를 입력해 주세요">
                    <label for="loginemail">이메일</label>
                    <input type="text" id="loginemail"  name="userEmail" placeholder="이메일을 입력해 주세요">
                    <br>
                    <!--데이터를 서버로 전송-->
                    <button type="button" onclick="findPwd_Proc()" >비밀번호 찾기</button>
                    <br>
	...
	function findPwd_Proc(){ 
	 ...
	 /*       alert("findId_Proc  1");  */
	          ...
	          {
	        	  type : "post",
	        	 url : "<%= contextPath%>/findPwd_Proc.me",    
	        	 data : {checkId : $userId.val(),  
	        		     checkEmail : $userEmail.val()},
	        	 success : function(result)  
	        	 {
	        		/*  alert("findPwd_Proc  success   result "  +  result);   */
	        		  
	        		 if(result == "0")
	        		 {
	        			 alert("회원정보에 없는 비밀번호 입니다.");
	        			   $userName.focus();  
	        	     } 
	        		 else
	        	     {
	        			 alert("회원님에 비밀번호는 " + result +  " 입니다.");
	        			 $userName.focus();
